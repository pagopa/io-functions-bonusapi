...
import { FindBonusActivationTaskT } from "./";
...
export const getRetrieveBonusActivityHandler = (
  findBonusActivationTask: ReturnType<FindBonusActivationTaskT>
) => {
  return async (context: Context, input: unknown): Promise<ActivityResult> => {
    const logPrefix = "RetrieveBonusActivity";

    context.log.info(`${logPrefix}|INFO|Input: ${input}`);
    ...
    return await fromEither(
      ...
      .chain(({ bonusID }) => findBonusActivationTask(bonusID))
